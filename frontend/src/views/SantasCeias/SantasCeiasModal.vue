...
<template>
              ...
              <DialogPanel class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-[700px] sm:w-full">
                ...
                <form @submit.prevent="onSubmit">
                  <div class="bg-white px-4 pt-5 pb-4">
                    <flat-pickr v-model="santaCeia.createdAt" class="block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm rounded-md" 
                    :config="{
                      enableTime: true,
  ...
  <script setup>
    ...
    const santaCeia = ref({
      id: props.santaCeia.id,
      createdAt: props.santaCeia.createdAt
    })
    ...
    function closeModal() {
      show.value = false
      emit('close', santaCeia.value)
    }
  
    function onSubmit() {
      loading.value = true
      store.dispatch('updateSantaCeia', santaCeia.value)
        .then(response => {
          loading.value = false;
