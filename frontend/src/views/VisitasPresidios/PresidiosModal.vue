...
<template>
              ...
              <DialogPanel class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-[700px] sm:w-full">
                ...
                <header class="py-3 px-4 flex justify-between items-center">
                  <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                    {{`Atualizar visita ao presidio: "${props.visitaPresidio.nome ? props.visitaPresidio.nome : 'Sem nome'}"`}}
                  </DialogTitle>
                ...
                <form @submit.prevent="onSubmit">
                  <div class="bg-white px-4 pt-5 pb-4">
                    <CustomInput class="mb-2" v-model="visitaPresidio.nome" label="Nome do visitado"/>
                    <flat-pickr v-model="visitaPresidio.createdAt" class="block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm rounded-md" 
                    :config="{
                      enableTime: true,
  ...
  <script setup>
    ...
    const visitaPresidio = ref({
      id: props.visitaPresidio.id,
      nome: props.visitaPresidio.nome,
      createdAt: props.visitaPresidio.createdAt
    })
    ...
    function closeModal() {
      show.value = false
      emit('close', visitaPresidio.value)
    }
  
    function onSubmit() {
      loading.value = true
      store.dispatch('updateVisitaPresidio', visitaPresidio.value)
        .then(response => {
          loading.value = false;
