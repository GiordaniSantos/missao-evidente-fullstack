  ...
  <div class="bg-white p-4 rounded-lg shadow animate-fade-in-down">
    <div class="flex justify-between border-b-2 pb-3">
      <div class="flex items-center">
        ...
        <select @change="getVisitasCrentes(null)" v-model="perPage" class="appearance-none relative block w-24 px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
        ...
        <span class="ml-3">Encontrado {{visitasCrentes.total}} Visitas aos Crentes</span>
      </div>
      <div>
        <input v-model="search" @change="getVisitasCrentes(null)" class="appearance-none relative block w-48 px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Procurar Visitas aos Crentes">
      </div>
    </div>
    <table class="table-auto w-full">
      <thead>
        <tr>
          ...
          <TableHeaderCell field="createdAt" :sort-field="sortField" :sort-direction="sortDirection" @click="sortVisitasCrentes('createdAt')">
            Data da Visita
          </TableHeaderCell>
          <TableHeaderCell field="nome" :sort-field="sortField" :sort-direction="sortDirection" @click="sortVisitasCrentes('nome')">
            Nome do Visitado
          </TableHeaderCell>
      ...
      <tbody v-if="visitasCrentes.loading || !visitasCrentes.data.length">
        <tr>
          <td colspan="6">
            <Spinner v-if="visitasCrentes.loading"/>
                ...
                <MenuItems class="absolute z-10 right-0 mt-2 w-32 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                  <div class="px-1 py-1">
                    <MenuItem v-slot="{ active }">
                      <button :class="[ active ? 'bg-indigo-600 text-white' : 'text-gray-900', 'group flex w-full items-center rounded-md px-2 py-2 text-sm',]" @click="editVisitaCrente(visitaCrente)">
                        <PencilIcon :active="active" class="mr-2 h-5 w-5 text-indigo-400" aria-hidden="true"/>
                        Editar
                      </button>
                    </MenuItem>
                    <MenuItem v-slot="{ active }">
                      <button :class="[active ? 'bg-indigo-600 text-white' : 'text-gray-900','group flex w-full items-center rounded-md px-2 py-2 text-sm',]" @click="deleteVisitaCrente(visitaCrente)">
    ...
    <div v-if="!visitasCrentes.loading" class="flex justify-between items-center mt-5">
      ...
      <nav v-if="visitasCrentes.total > visitasCrentes.limit" class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px" aria-label="Pagination">
        ...
        <a
          v-for="(link, i) of visitasCrentes.links"
          :key="i"
          :disabled="!link.url"
          href="#"
          ...
          :class="[
              ...
              i === 0 ? 'rounded-l-md' : '',
              i === visitasCrentes.links.length - 1 ? 'rounded-r-md' : '',
...
<script setup>
  ...
  const showVisitaCrenteModal = ref(false);
  ...
  function getForPage(ev, link) {
    ...
    getVisitasCrentes(link.url)
  }

  function getVisitasCrentes(url = null) {
  ...
  function sortVisitasCrentes(field) {
  ...
  function showAddNewModal() {
    showVisitaCrenteModal.value = true
  }

  function deleteVisitaCrente(visitaCrente) {
    Swal.fire({
      ...
      icon: 'warning'
        ...
        if (result.isConfirmed) {
          store.dispatch('deleteVisitaCrente', visitaCrente.id)
          .then(res => {
