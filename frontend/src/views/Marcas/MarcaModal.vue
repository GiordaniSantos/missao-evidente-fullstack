            ...
            <DialogPanel class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-[700px] sm:w-full">
              ...
              <header class="py-3 px-4 flex justify-between items-center">
                <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                  {{ marca.id ? `Atualizar Marca: "${props.marca.nome}"` : 'Criar novo Marca' }}
                </DialogTitle>
...
<script setup>
  ...
  function onSubmit() {
    loading.value = true
    if (marca.value.id) {
      store.dispatch('updateMarca', marca.value)
        .then(response => {
          loading.value = false;
      ...
      store.dispatch('createMarca', marca.value)
        .then(response => {
          loading.value = false;
