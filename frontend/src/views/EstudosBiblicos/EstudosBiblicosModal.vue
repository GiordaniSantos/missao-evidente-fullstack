...
<template>
              ...
              <DialogPanel class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-[700px] sm:w-full">
                ...
                <header class="py-3 px-4 flex justify-between items-center">
                  <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900">
                    {{`Atualizar Estudo Biblico: "${props.estudoBiblico.nome ? props.estudoBiblico.nome : 'Sem assunto'}"`}}
                  </DialogTitle>
                ...
                <form @submit.prevent="onSubmit">
                  <div class="bg-white px-4 pt-5 pb-4">
                    <CustomInput class="mb-2" v-model="estudoBiblico.nome" label="Assunto"/>
                    <flat-pickr v-model="estudoBiblico.createdAt" class="block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm rounded-md" 
                    :config="{
                      enableTime: true,
  ...
  <script setup>
    ...
    const estudoBiblico = ref({
      id: props.estudoBiblico.id,
      nome: props.estudoBiblico.nome,
      createdAt: props.estudoBiblico.createdAt
    })
    ...
    function closeModal() {
      show.value = false
      emit('close', estudoBiblico.value)
    }
  
    function onSubmit() {
      loading.value = true
      store.dispatch('updateEstudoBiblico', estudoBiblico.value)
        .then(response => {
          loading.value = false;
