<template>
    <div class="bg-white p-4 rounded-lg shadow animate-fade-in-down">
      <div class="flex justify-between border-b-2 pb-3">
        <div class="flex items-center">
          ...
          <select @change="getDiscipulados(null)" v-model="perPage" class="appearance-none relative block w-24 px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
      ...
      <table class="table-auto w-full">
        <thead>
          <tr>
            <TableHeaderCell field="id" :sort-field="sortField" :sort-direction="sortDirection" @click="sortDiscipulados('id')">
              ID
            </TableHeaderCell>
        ...
        <tbody v-if="discipulados.loading || !discipulados.data.length">
          <tr>
            <td colspan="6">
              <Spinner v-if="discipulados.loading"/>
                  ...
                  <MenuItems class="absolute z-10 right-0 mt-2 w-32 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div class="px-1 py-1">
                      <MenuItem v-slot="{ active }">
                        <button :class="[ active ? 'bg-indigo-600 text-white' : 'text-gray-900', 'group flex w-full items-center rounded-md px-2 py-2 text-sm',]" @click="editDiscipulado(discipulado)">
                          <PencilIcon :active="active" class="mr-2 h-5 w-5 text-indigo-400" aria-hidden="true"/>
                          Editar
                        </button>
                      </MenuItem>
                      <MenuItem v-slot="{ active }">
                        <button :class="[active ? 'bg-indigo-600 text-white' : 'text-gray-900','group flex w-full items-center rounded-md px-2 py-2 text-sm',]" @click="deleteDiscipulado(discipulado)">
      ...
      <div v-if="!discipulados.loading" class="flex justify-between items-center mt-5">
        <div v-if="discipulados.data.length">
          Mostrando página {{ discipulados.page }} de {{ discipulados.totalPage }}
        </div>
        <nav v-if="discipulados.total > discipulados.limit" class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px" aria-label="Pagination">
          <!-- Current: "z-10 bg-indigo-50 border-indigo-500 text-indigo-600", Default: "bg-white border-gray-300 text-gray-500 hover:bg-gray-50" -->
          <a
            v-for="(link, i) of discipulados.links"
            :key="i"
            :disabled="!link.url"
            href="#"
            ...
            :class="[
                ...
                i === 0 ? 'rounded-l-md' : '',
                i === discipulados.links.length - 1 ? 'rounded-r-md' : '',
  ...
  <script setup>
    ...
    //import discipuladoModal from "./discipuladoModal.vue";
    import Swal from 'sweetalert2'
  
    const perPage = ref(ITENS_PER_PAGE);
    const search = ref('');
    const discipulados = computed(() => store.state.discipulados);
    const sortField = ref('updatedAt');
    const sortDirection = ref('desc')
  
    const discipulado = ref({})
    const showDiscipuladoModal = ref(false);
    ...
    function getForPage(ev, link) {
      ...
      getDiscipulados(link.url)
    }
  
    function getDiscipulados(url = null) {
    ...
    function sortDiscipulados(field) {
    ...
    function showAddNewModal() {
      showDiscipuladoModal.value = true
    }
  
    function deleteDiscipulado(discipulado) {
      Swal.fire({
        ...
        icon: 'warning'
          ...
          if (result.isConfirmed) {
            store.dispatch('deleteDiscipulado', discipulado.id)
            .then(res => {
